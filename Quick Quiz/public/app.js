...
function setMode(mode) {
  ...
  if (host && state.sessionCode && state.hostToken && !state.hostViewState) {
    connectHostSocket(state.sessionCode, state.hostToken);
  }
}
...
function resetToHome(targetMode = "host") {
  stopCountdown();
  if (state.hostSocket) {
    state.hostSocket.close();
    state.hostSocket = null;
  }
  if (state.participantSocket) {
    state.participantSocket.close();
    state.participantSocket = null;
  }
...
function connectHostSocket(code, hostToken) {
  if (state.hostSocket) {
    state.hostSocket.close();
  }

  const wsProtocol = location.protocol === "https:" ? "wss:" : "ws:";
  state.hostSocket = new WebSocket(
    `${wsProtocol}//${location.host}${buildAppPath("/")}?role=host&code=${encodeURIComponent(code)}&hostToken=${encodeURIComponent(hostToken)}`
  );
  state.hostSocket.addEventListener("message", (event) => {
    const payload = JSON.parse(event.data);
    if (payload.type === "state") {
      state.hostViewState = payload.state;
      renderHostState();
    }
  });
  state.hostSocket.addEventListener("close", () => {
    if (state.hostSocket?.readyState === WebSocket.CLOSED) {
      state.hostSocket = null;
    }
  });
}

function connectParticipantSocket(code, participantId) {
  if (state.participantSocket) {
    state.participantSocket.close();
  }

  const wsProtocol = location.protocol === "https:" ? "wss:" : "ws:";
  state.participantSocket = new WebSocket(
    `${wsProtocol}//${location.host}${buildAppPath("/")}?role=participant&code=${encodeURIComponent(code)}&participantId=${encodeURIComponent(participantId)}`
  );
  state.participantSocket.addEventListener("message", (event) => {
    const payload = JSON.parse(event.data);
    if (payload.type === "state") {
      state.participantViewState = payload.state;
...
async function handleCreateSession() {
  try {
    ...
    connectHostSocket(result.code, result.hostToken);
...
elements.joinButton.addEventListener("click", async () => {
  try {
    ...
    connectParticipantSocket(code, result.participantId);
...
function restoreParticipantFromUrl() {
  ...
  if (state.participantId && state.participantCode) {
    ...
    connectParticipantSocket(state.participantCode, state.participantId);
  }
